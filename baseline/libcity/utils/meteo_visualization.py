"""
气象预测可视化工具
- 与 HyperGKAN 的可视化格式完全对齐
- 支持预测曲线图 (样本 x 站点 网格)
- 支持综合分析图 (散点图、误差分布、MAE柱状图、箱线图、最佳/最差样例)
"""
import matplotlib
matplotlib.use('Agg')  # 非交互式后端，避免 GUI 相关问题
...
def _select_representative_samples(
    ...
    selected = set()
    selected.add(sorted_indices[0])  # 最佳
    if B > 1:
        selected.add(sorted_indices[max(1, B // 10)])  # 次佳(10%分位)
    if B > 2:
        selected.add(sorted_indices[B // 2])  # 中位数
    if B > 3:
        selected.add(sorted_indices[min(B - 1, int(B * 0.9))])  # 较差(90%分位)
    ...
    return sorted(list(selected))[:num_samples]
...
def _select_diverse_stations(
    ...
    return sorted(list(selected))[:num_stations]
...
def plot_predictions(
    ...
    Args:
        pred: 预测值 (B, T, N, F)，已在原始尺度
        target: 真实值 (B, T, N, F)，已在原始尺度
        save_path: 保存路径
    ...
    for row_idx, si in enumerate(sample_indices):
        for col_idx, sj in enumerate(station_indices):
            ...
            pred_series = pred[si, :, sj, 0]
            target_series = target[si, :, sj, 0]
    ...
    fig.suptitle(
        f'{model_name} {element} Prediction  (Overall MAE={overall_mae:.3f}{unit})',
    ...
    horizon_mae = []
    for t in range(T):
        h_mae = np.mean(np.abs(pred[:, t, :, 0] - target[:, t, :, 0]))
    ...
    station_maes = np.mean(np.abs(pred[:, :, :, 0] - target[:, :, :, 0]),
                           axis=(0, 1))  # (N,)
    ...
    # --- (e) 最佳样本+站点展示 ---
    pair_mae = np.mean(np.abs(pred[:, :, :, 0] - target[:, :, :, 0]),
                       axis=1)  # (B, N)
    ...
    pred_best = pred[best_si, :, best_sj, 0]
    target_best = target[best_si, :, best_sj, 0]
    ...
    pred_worst = pred[worst_si, :, worst_sj, 0]
    target_worst = target[worst_si, :, worst_sj, 0]
    ...
    overall_rmse = np.sqrt(np.mean((pred - target) ** 2))
    fig2.suptitle(
        f'{model_name} {element} Prediction Analysis  |  Overall MAE={overall_mae:.3f}{unit}  '
        f'RMSE={overall_rmse:.3f}{unit}',
        fontsize=14, fontweight='bold', y=1.01
    )
...
def plot_loss_curve(
    ...
    plt.title(f'{model_name} on {element} - Loss Curve', fontsize=14, fontweight='bold')
