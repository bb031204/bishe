...
class StateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        if os.path.exists(self.data_path + self.geo_file + '.geo'):
            self._load_geo()
        elif os.path.exists(self.data_path + self.geo_file + '_adj.csv'):
            self.load_adj()
        else:
    ...
    def load_adj(self):
        ...
        加载（N*N）的邻接矩阵
        """
        self.adj_mx = pd.read_csv(self.data_path + self.geo_file + '_adj.csv', header=None).to_numpy()
    ...
    def _load_rel(self):
        ...
        # adj_data = load_data('/home/fjt/Bigscity-LibCity-master/raw_data/humidity/trn.pkl')
        
        # self.adj_mx = np.eyes(adj_data['x'].shape[2],adj_data['x'].shape[2])
        # 计算权重
        if self.distance_inverse and self.set_weight_link_or_dist.lower() != 'link':
            ...
            print('calculate_adjacency_matrix')
    ...
    def _calculate_adjacency_matrix(self):
        ...
        # self.adj_mx = np.eye(adj_data['x'].shape[2],adj_data['x'].shape[2])
        # print(self.adj_mx.shape)
    ...
    def _load_dyna_3d(self, filename):
        ...
        """
        # 加载数据集
        if os.path.exists(self.data_path + filename + '.dyna'):
        ...
        elif os.path.exists(self.data_path + filename + '_feature.csv'):
            data = pd.read_csv(self.data_path + filename + '_feature.csv', header=None).to_numpy()
            data = np.expand_dims(data,-1)
            self._logger.info("Loaded file " + filename + '_feature.csv' + ', shape=' + str(data.shape))
        return data
